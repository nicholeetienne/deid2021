...
#
#deid_sarthak_main.py : De-identification algorithm -- scrubs Date PHI from free-text medical records 
# Command to run the python program: 
# python deid_sarthak_main.py <input_filename> <output_filename>
#
# Input arguments: 
...
# python stats.py id.deid id-phi.phrase <output file from deid_sarthak_main.py script
...
def check_for_date(patient,note,chunk, output_handle):
    ...
    Logic:
        ...
        Use the precompiled regular expression from Block 1 to find dates.
    """
    ...
    for match in date_reg.finditer(chunk): 
        #### the regular expression is matched in the chunk to get their location with reference to the chunk start
